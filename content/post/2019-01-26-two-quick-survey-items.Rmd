...
where $\pi^*_{ij}$ is the pairwise inclusion version of $\pi_i^*$.
...
$$\check{\Delta}=\check{\Delta}_1+\check{\Delta}_2-\check{\Delta}_1\check{\Delta}_2.$$
...
The last form is especially useful, because when we're working with a particular stage of sampling we're going to have the probabilities at that stage and the sample size at that stage conveniently available, but we might not have $N$ conveniently available.  