...
There are various reasons why nested models might fail to be symbolically nested. A simple one is that the variables have different names: if you have one observation per day, then `day` and `date` are the same variable.  Another is transformations that lose information: a model with `day_of_the_week` and a model with an indicator variable `weekend` are nested. Another is variable expansion: a linear term in a numeric variable and a regression spline are nested. 
