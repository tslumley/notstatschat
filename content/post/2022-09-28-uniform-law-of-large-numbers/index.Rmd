...
Theorem: Let ${\cal F}$ be a class of measurable functions such that (i) $f(X)$ satisfies a weak law of large numbers for each $f\in{\cal F}$ and (ii) the bracketing numbers $N_{[\,]}(\epsilon,\|\cdot\|,{\cal F})$ are finite for every $\epsilon>0$. The law of large numbers holds uniformly over $f\in{\cal F}$. 
