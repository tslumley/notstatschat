...
If you think about an EM-type algorithm, the M-step for estimating $\beta$ and $\sigma^2$ should use the school weights in the first term (and it doesn't affect $\tau^2$) but the E-step for estimating the $u_i$ should use something more like the sample size in the school than the population size in the school. Of course, that would stop the algorithm being the EM algorithm. It might still work, but we're getting away from nice principled design-weighted inference into {\em ad hoc} algorithm fiddling.   
